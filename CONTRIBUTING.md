...
* Open an Issue at https://github.com/Scout-NU/mass-hike-api/issues
...
* Go to https://github.com/Scout-NU/mass-hike-api/pulls and open a new pull request with your changes.
...
* If your PR gets accepted, it will be marked as such, and merged into the `develop` branch soon after. Your contribution will be distributed to the masses next time the maintainers [tag a release](#tag-a-release)
...
* Dependency updates, additions, or removals must be in individual commits, and the message must use the format: `deps: update PKG@VERSION`.
* Go to https://github.com/Scout-NU/mass-hike-api/pulls and open a new pull request with your changes.
* If your PR is connected to an open issue, add a line in your PR's description that says `Fixes: #123`, where `#123` is the number of the issue you're fixing.
...
* Go to the issue tracker and [filter open issues by the `support` label](https://github.com/Scout-NU/mass-hike-api/issues?q=is%3Aopen+is%3Aissue+label%3Asupport).
...
In order to label issues, [open up the list of unlabeled issues](https://github.com/Scout-NU/mass-hike-api/issues?q=is%3Aopen+is%3Aissue+no%3Alabel) and, **from newest to oldest**, read through each one and apply issue labels according to the table below. If you're unsure about what label to apply, skip the issue and try the next one: don't feel obligated to label each and every issue yourself!
...
* Leave a message when closing saying "Cleaning up stale issue. Please reopen or ping us if and when you're ready to resume this. See https://github.com/Scout-NU/mass-hike-api/blob/latest/CONTRIBUTING.md#clean-up-issues-and-prs for more details."
...
Pull requests will only be merged once all checks have passed and all pull request reviews have an approved status. In the most special of circumstances, an administrator may override this and merge a PR prematurely.

## Tag A Release

All releases for this project will use [semver](https://semver.org). The active development branch for the project will always be the `develop` branch.

When it's time to cut a new release, create a new commit incrementing the version number in all relevant places.

If the version number is a major version increase, before merging to the release branch, a new branch named `[MAJOR].x.x` for the **previous** major version should be created from the latest master state. This will preserve old major versions in the event that security patches or bug fixes must be made to old project versions.

Once the version number has been incremented and a new maintenance branch has been created, if needed, the current `develop` branch will be tagged with the new release number, merged into the `master` branch, and then deployed to npm. `master` should always reflect the latest deployed version of the project.
